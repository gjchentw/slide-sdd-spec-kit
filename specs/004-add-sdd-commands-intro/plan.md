# 實作計畫：新增 Spec Kit 指令與 SDD 流程對照投影片

**分支**: `004-add-sdd-commands-intro` | **日期**: 2026-02-18 | **規格文件**: [specs/004-add-sdd-commands-intro/spec.md](spec.md)
**輸入**: 來自 `/specs/004-add-sdd-commands-intro/spec.md` 的功能規格

**備註**: 此範本由 `/speckit.plan` 指令填寫。

## 摘要

本功能將新增一組投影片，介紹 Spec Kit 指令如何對應 SDD 開發流程。
內容涵蓋 6 個開發階段（憲章、啟動、提煉、計畫、拆解、執行）與對應指令。
將建立獨立的 Markdown 檔案 `slides/03-sdd-commands.md`，並於 `index.html` 中載入。
其中將包含 Mermaid 流程圖以視覺化展示指令流向。

## 技術背景

**語言/版本**: HTML5, JavaScript (reveal.js 5.x)
**主要依賴**: reveal.js, reveal.js-mermaid-plugin
**儲存**: Markdown files (`slides/03-sdd-commands.md`)
...
```text
specs/004-add-sdd-commands-intro/
├── plan.md              # 此檔案
├── research.md          # 階段 0 輸出
